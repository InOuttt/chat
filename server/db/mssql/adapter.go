...
func (a *adapter) IsOpen() bool {
	af.Log.Info("[ adapter isopen ]")
	return a.db != nil
}

// GetDbVersion returns current database version.
func (a *adapter) GetDbVersion() (int, error) {
	af.Log.Info("[ adapter getdbversion ]")
	if a.version > 0 {
...
func (a *adapter) AuthDelAllRecords(user t.Uid) (int, error) {
	af.Log.Info("[ adapter authdelallrecords ]")
...
func (a *adapter) AuthGetRecord(uid t.Uid, scheme string) (string, auth.Level, []byte, time.Time, error) {
	af.Log.Info("[ adapter authgetrecord ]")
...
func (a *adapter) AuthGetUniqueRecord(unique string) (t.Uid, auth.Level, []byte, time.Time, error) {
	af.Log.Info("[ adapter authgetuniquerecord ]")
	var expires time.Time
...
func (a *adapter) UserGetAll(ids ...t.Uid) ([]t.User, error) {
	af.Log.Info("[ adapter usergetall ]")
...
func (a *adapter) UserGetByCred(method, value string) (t.Uid, error) {
	af.Log.Info("[ adapter usergetbycred ]")
...
func (a *adapter) UserUnreadCount(uid t.Uid) (int, error) {
	af.Log.Info("[ adapter userunreadcount ]")
	var count int
...
func (a *adapter) TopicsForUser(uid t.Uid, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	af.Log.Info("[ adapter topicsforuser ]")
	...
	if len(topq) > 0 {
		...
		q, topq, _ := sqlx.In(
			"SELECT [createdat],[updatedat],[deletedat],[touchedat],[name] AS [id],[access],[seqid],[delid],[public],[tags] "+
				"FROM [dbo].[topics] WHERE [name] IN (?)", topq)
		q = a.db.Rebind(q)
	...
	if err == nil && len(usrq) > 0 {
		q, usrq, _ := sqlx.In(
			"SELECT [id],[state],[createdat],[updatedat],[deletedat],[access],[lastseen],[useragent],[public],[tags] FROM [dbo].[users] WHERE [id] IN (?)",
			usrq)
...
func (a *adapter) UsersForTopic(topic string, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	af.Log.Info("[ adapter usersfortopic ]")
...
func (a *adapter) SubscriptionGet(topic string, user t.Uid) (*t.Subscription, error) {
	af.Log.Info("[ adapter subscriptionget ]")
	var sub t.Subscription
...
func (a *adapter) SubsLastSeen(topic string, user t.Uid, lastSeen map[string]time.Time) error {
	af.Log.Info("[ adapter subslastseen ]")
...
func (a *adapter) SubsForUser(forUser t.Uid, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	af.Log.Info("[ adapter subsforuser ]")
...
func (a *adapter) SubsForTopic(topic string, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	af.Log.Info("[ adapter subsfortopic ]")
	q := `SELECT [createdat],[updatedat],[deletedat],[userid] AS [user],[topic],[delid],[recvseqid],
...
func (a *adapter) SubsDelete(topic string, user t.Uid) error {
	af.Log.Info("[ adapter subsdelete ]")
	now := t.TimeNow()
...
func (a *adapter) SubsDelForTopic(topic string, hard bool) error {
	af.Log.Info("[ adapter subsdelfortopic ]")
	var err error
	if hard {
...
func (a *adapter) FindUsers(uid t.Uid, req, opt []string) ([]t.Subscription, error) {
	...
	FROM
		[dbo].[users] AS [u]
		LEFT JOIN [dbo].[usertags] AS [t] ON [t].[userid]= [u].[id]
	WHERE
		[t].[tag] IN ( 'email:test1@mailinator.com' )
	...
	query := `SELECT
		...
		FROM
			...
			LEFT JOIN [dbo].[usertags] AS [t] ON [t].[userid]= [u].[id]
		WHERE
			[t].[tag] IN (?` + strings.Repeat(",?", len(req)+len(opt)-1) + ") AND [u].[deletedat] IS NULL " +
		`GROUP BY
			[u].[id],
			[u].[createdat],
			[u].[updatedat],
			...
			[u].[tags] `
	if len(req) > 0 {
		query += "HAVING COUNT ( CASE WHEN [t].[tag] IN (?" + strings.Repeat(",?", len(req)-1) + ") OR [t].[tag] IS NULL THEN 1 ELSE 0 END ) >=? "
		for _, tag := range req {
			args = append(args, tag)
		}
	...
	// af.Log.Ln(args, a.maxResults)
	// log.Printf(ErrorColor, query)
...
func (a *adapter) FindTopics(req, opt []string) ([]t.Subscription, error) {
	af.Log.Info("[ adapter findtopics ]")
	index := make(map[string]struct{})
	var args []interface{}
	for _, tag := range append(req, opt...) {
		args = append(args, tag)
	...
	query := "SELECT [t].[name] AS [topic],[t].[createdat],[t].[updatedat],MAX([t].[access]) AS [access],[t].[public],[t].[tags],COUNT(*) AS [matches] " +
	...
	if len(req) > 0 {
		query += "HAVING COUNT(CASE WHEN [tt].[tag] IN (?" + strings.Repeat(",?", len(req)-1) + ") OR [tt].[tag] IS NULL THEN 1 ELSE 0 END)>=? "
...
func (a *adapter) MessageSave(msg *t.Message) error {
	af.Log.Info("[ adapter messagesave ]")
	var res sql.Result
	var err error
...
func (a *adapter) MessageAttachments(msgId t.Uid, fids []string) error {
	af.Log.Info("[ adapter messageattachments ]")
	var args []interface{}
	var values []string
...
func (a *adapter) DeviceUpsert(uid t.Uid, def *t.DeviceDef) error {
	af.Log.Info("[ adapter deviceupsert ]")
...
func (a *adapter) DeviceGetAll(uids ...t.Uid) (map[t.Uid][]t.DeviceDef, int, error) {
	af.Log.Info("[ adapter devicegetall ]")
	var unums []interface{}
...
func (a *adapter) CredUpsert(cred *t.Credential) (bool, error) {
	af.Log.Info("[ adapter credupsert ]")
	var err error
...
func (a *adapter) CredIsConfirmed(uid t.Uid, method string) (bool, error) {
	af.Log.Info("[ adapter credisconfirmed ]")
	var done int
...
func (a *adapter) CredConfirm(uid t.Uid, method string) error {
	af.Log.Info("[ adapter credconfirm ]")
	res, err := a.db.Exec(
...
func (a *adapter) CredGetActive(uid t.Uid, method string) (*t.Credential, error) {
	af.Log.Info("[ adapter credgetactive ]")
...
func (a *adapter) CredGetAll(uid t.Uid, method string, validatedOnly bool) ([]t.Credential, error) {
	af.Log.Info("[ adapter credgetall ]")
...
func (a *adapter) FileStartUpload(fd *t.FileDef) error {
	af.Log.Info("[ adapter filestartupload ]")
...
func (a *adapter) FileFinishUpload(fid string, status int, size int64) (*t.FileDef, error) {
	af.Log.Info("[ adapter filefinishupload ]")
	id := t.ParseUid(fid)
	if id.IsZero() {
